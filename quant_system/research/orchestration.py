...
def run_symbol_research_orchestration(
    ...
    *,
    ...
    lines = [
        ...
        f"Prop broker: {config.mt5.prop_broker}",
        f"Market data fetch policy: {config.market_data.fetch_policy}",
        f"Market data DB: {config.mt5.database_path}",
        f"Research source preference: {config.symbol_research.source_preference}",
        f"Data source: {data_source}",
        f"Broker data source: {broker_data_summary.get('broker_data_source', data_source)}",
        f"Broker history bars loaded: {int(broker_data_summary.get('history_bars_loaded', 0) or 0)}",
        f"Broker requested bars: {int(broker_data_summary.get('requested_bars', 0) or 0)}",
        f"Broker single-request cap: {int(broker_data_summary.get('safe_single_request_bars', 0) or 0)}",
        f"Broker terminal maxbars: {int(broker_data_summary.get('terminal_maxbars', 0) or 0)}",
        f"Broker backfill batch bars: {int(broker_data_summary.get('batch_bars', 0) or 0)}",
        f"Broker backfill batches used: {int(broker_data_summary.get('batch_count', 0) or 0)}",
        f"Broker history complete: {bool(broker_data_summary.get('history_complete', False))}",
