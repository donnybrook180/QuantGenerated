...
class ResearchEndToEndTests(unittest.TestCase):
    def test_research_end_to_end_selects_intraday_candidate_for_forex_symbol(self) -> None:
        ...
        self.assertTrue(any(line.startswith("Market data fetch policy: ") for line in lines))
        self.assertIn("Market data DB: quant_data_blue_guardian.duckdb", lines)
        self.assertTrue(any(line.startswith("Broker requested bars: ") for line in lines))
        self.assertTrue(any(line.startswith("Broker single-request cap: ") for line in lines))
        self.assertTrue(any(line.startswith("Broker backfill batches used: ") for line in lines))
