        ...
        return [
            {
                ...
                "best_trade_share_pct": float(row[22] or 0.0),
                "equity_new_high_share_pct": float(row[23] or 0.0),
                "max_consecutive_losses": int(row[24] or 0),
                "equity_quality_score": float(row[25] or 0.0),
                "dominant_exit": row[26] or "",
                "dominant_exit_share_pct": float(row[27] or 0.0),
                "walk_forward_windows": int(row[28] or 0),
                "walk_forward_pass_rate_pct": float(row[29] or 0.0),
                "walk_forward_avg_validation_pnl": float(row[30] or 0.0),
                "walk_forward_avg_test_pnl": float(row[31] or 0.0),
                "walk_forward_avg_validation_pf": float(row[32] or 0.0),
                "walk_forward_avg_test_pf": float(row[33] or 0.0),
                "component_count": int(row[34] or 1),
                "combo_outperformance_score": float(row[35] or 0.0),
                "combo_trade_overlap_pct": float(row[36] or 0.0),
                "best_regime": row[37] or "",
                "best_regime_pnl": float(row[38] or 0.0),
                "worst_regime": row[39] or "",
                "worst_regime_pnl": float(row[40] or 0.0),
                "dominant_regime_share_pct": float(row[41] or 0.0),
                "regime_stability_score": float(row[42] or 0.0),
                "regime_loss_ratio": float(row[43] or 0.0),
                "regime_trade_count_by_label": row[44] or "{}",
                "regime_pnl_by_label": row[45] or "{}",
                "regime_pf_by_label": row[46] or "{}",
                "regime_win_rate_by_label": row[47] or "{}",
                "variant_label": row[48] or "",
                "timeframe_label": row[49] or "",
                "session_label": row[50] or "",
                "regime_filter_label": row[51] or "",
                "execution_overrides": json.loads(row[52] or "{}"),
                "recommended": bool(row[53]),
                "profile_name": row[54],
            }
            for row in rows
        ]
