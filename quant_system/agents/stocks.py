...
class EventAwareRiskSentinelAgent(Agent):
    ...
    def __init__(self, allow_high_impact_day: bool = False) -> None:
...
class StockTrendBreakoutAgent(Agent):
    ...
    def __init__(self, fast_window: int = 10, slow_window: int = 30, min_relative_volume: float = 1.0) -> None:
    ...
    def on_feature(self, feature: FeatureVector) -> SignalEvent | None:
        ...
        if (
            ...
            or feature.values.get("news_count_1d", 0.0) > 5.0
        ):
        ...
        if fast > slow and momentum_5 > 0 and momentum_20 > 0 and atr_proxy >= 0.003:
            return SignalEvent(
                feature.timestamp,
                self.name,
                feature.symbol,
...
class StockGapAndGoAgent(Agent):
    ...
    def __init__(self, min_relative_volume: float = 1.2, min_atr_proxy: float = 0.0035) -> None:
        self.min_relative_volume = min_relative_volume
        self.min_atr_proxy = min_atr_proxy
    ...
    def on_feature(self, feature: FeatureVector) -> SignalEvent | None:
        if feature.values.get("in_regular_session", 0.0) < 1.0:
            return SignalEvent(feature.timestamp, self.name, feature.symbol, Side.FLAT, 0.0, {})
        minutes_from_open = feature.values.get("minutes_from_open", -1.0)
        if minutes_from_open < 5 or minutes_from_open > 90:
            return SignalEvent(feature.timestamp, self.name, feature.symbol, Side.FLAT, 0.0, {})
        if feature.values.get("event_blackout", 0.0) >= 1.0:
            return SignalEvent(feature.timestamp, self.name, feature.symbol, Side.FLAT, 0.0, {})
        rel_vol = feature.values.get("relative_volume", 1.0)
        ...
        if rel_vol < self.min_relative_volume or atr_proxy < self.min_atr_proxy:
            return SignalEvent(feature.timestamp, self.name, feature.symbol, Side.FLAT, 0.0, {})
        if momentum_5 > 0 and momentum_20 > 0 and trend_strength > 0:
            side = Side.BUY
        elif momentum_5 < 0 and momentum_20 < 0 and trend_strength < 0:
            side = Side.SELL
        else:
            side = Side.FLAT
        return SignalEvent(
...
class StockPowerHourContinuationAgent(Agent):
    ...
    def __init__(self, min_relative_volume: float = 0.95, min_momentum_20: float = 0.003) -> None:
        self.min_relative_volume = min_relative_volume
    ...
    def on_feature(self, feature: FeatureVector) -> SignalEvent | None:
        ...
        if rel_vol < self.min_relative_volume:
            return SignalEvent(feature.timestamp, self.name, feature.symbol, Side.FLAT, 0.0, {})
        if momentum_5 > 0 and momentum_20 >= self.min_momentum_20 and trend_strength > 0:
            side = Side.BUY
        elif momentum_5 < 0 and momentum_20 <= -self.min_momentum_20 and trend_strength < 0:
            side = Side.SELL
        else:
            side = Side.FLAT
        return SignalEvent(
