...
class US500TrendPullbackAgent(Agent):
    ...
    def on_feature(self, feature: FeatureVector) -> SignalEvent | None:
        ...
        confidence = min(
            0.15 + (max(trend_strength, 0.0) * 160) + (max(momentum_20, 0.0) * 120) + (max(0.0, -z_score) * 0.12),
            1.0,
        )
        return SignalEvent(
...
class US500VWAPContinuationAgent(Agent):
    ...
    def on_feature(self, feature: FeatureVector) -> SignalEvent | None:
        ...
        confidence = min(
            0.2 + (max(trend_strength, 0.0) * 170) + (max(momentum_20, 0.0) * 110) + (max(momentum_5, 0.0) * 80),
            1.0,
        )
        return SignalEvent(
            timestamp=feature.timestamp,
...
class US500OpeningDriveReclaimAgent(Agent):
    ...
    def on_feature(self, feature: FeatureVector) -> SignalEvent | None:
        ...
        confidence = min(
            0.18 + (max(trend_strength, 0.0) * 180) + (max(momentum_20, 0.0) * 110) + (max(momentum_5, 0.0) * 90),
            1.0,
        )
        return SignalEvent(
            timestamp=feature.timestamp,
...
__all__ = ["US500TrendPullbackAgent", "US500VWAPContinuationAgent", "US500OpeningDriveReclaimAgent"]
